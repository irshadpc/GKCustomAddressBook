...
- (void)intialLoadAddressBook
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *contacts = self.preselectedContacts;
        ...
        for (GKContact *contact in contacts) {
            ...
            if ([contact.name isEqualToString:@""]) {
                identifier = [self identifierForName:[contact email]];
            } else {
                identifier = [self identifierForName:[contact name]];
            }
...
- (void)setupSearchBar
{
    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, 340, 44)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([contact.firstname length] == 0 && [contact.lastname length] == 0) {
        if ([contact.companyName isEqualToString:@""]) {
    ...
    //NSLog(@"%@ %@ %d",contact.name,[NSNumber numberWithBool:contact.isSelected],indexPath.row);
    cell.preselectedCell = (BOOL) contact.isSelected;
